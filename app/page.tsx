...
// ponytail: reading Worker vars via cloudflare:workers; process.env fallback covers `vinext dev` without workerd
export default function Home() {
  const hello = (env as unknown as Record<string, string>)?.HELLO_MESSAGE ?? process.env.HELLO_MESSAGE ?? "hello-from-worker";
  return (
    <main className="min-h-screen bg-slate-50 px-6 py-10 text-slate-950">
      <section className="mx-auto flex max-w-4xl flex-col gap-8">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="rounded-md border border-dashed border-orange-300 bg-white px-4 py-3 text-sm">
            <span className="font-semibold">Worker var:</span> <code>{hello}</code>
            <span className="ml-2 text-slate-500">(from wrangler.jsonc vars.HELLO_MESSAGE)</span>
          </div>
        </div>
