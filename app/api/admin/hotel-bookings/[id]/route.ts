import { updateHotelBookingStatusAdmin } from "@/lib/hotels";
...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    return Response.json(await updateHotelBookingStatusAdmin(id, parsed.data.status));
  } catch (err) {
    const status = (err as Error & { status?: number }).status ?? 500;
    if (status === 500)
      console.error("PUT /api/admin/hotel-bookings/[id] failed:", err instanceof Error ? err.message : err);
