...
export default function AdminBookingsPage() {
  ...
  const load = useCallback(async () => {
    try {
      const res = await adminFetch(getToken, `/bookings${filter ? `?status=${filter}` : ""}`);
  ...
  const setStatus = async (booking: AdminBooking, status: BookingStatus) => {
    setBusy(true);
    setError(null);
    try {
      const res = await adminFetch(getToken, `/bookings/${booking.id}`, {
        method: "PUT",
            ...
            <tbody className="divide-y">
              {bookings.map((b) => (
                <tr key={b.id} onClick={() => setSelected(b)} className="cursor-pointer hover:bg-muted/40">
                  ...
                  <td className="max-w-[200px] truncate px-4 py-2">{b.package_title}</td>
        ...
        <Dialog title={selected.booking_ref} onClose={() => setSelected(null)} locked={busy}>
          <dl className="space-y-2 text-sm">
            ...
            <div className="flex justify-between gap-4">
              ...
              <dd className="text-right">{selected.package_title}</dd>
            </div>
            <div className="flex justify-between gap-4">
            ...
            <div className="flex justify-between gap-4">
              <dt className="text-muted-foreground">Travelers / rooms</dt>
              <dd>
                {selected.adults + selected.children} / {selected.rooms}
              </dd>
            </div>
